...
class Feature8Builder(FeatureBuilderBase):
    def __init__(self, parser: MyParser, offset) -> None:
        tuples = parser.getTupleOfHeadAndModifier()
        tuples = sorted(list(set([(h.pos, m.token, m.pos) for (h, m) in tuples])))
    ...
    def getFeatureVector(self, history, head, modifier):
        ...
        tpl = head_token.pos, modifier_token.token, modifier_token.pos
